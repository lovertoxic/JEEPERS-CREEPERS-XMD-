...
ezra({
  ...
  if (!superUser) {
    return repondre("*This command is restricted to the bot owner. or Jeepers creepers owner* 🤦,,idiot");
  }
...
ezra({
  ...
  // Check if the command is issued by the owner
  if (!superUser) {
    return repondre("*This command is restricted to the bot owner or jeepers-creeper owner.* 🤦,,idiot");
  }
...
ezra({
  nomCom: 'autobio',
  categorie: "JEEPERS CREEPERS_XMD-SETTING"
}, async (chatId, zk, context) => {
  const { ms, repondre, superUser, auteurMessage, arg } = context;
