...
ezra({
  ...
  if (superUser || auteurMessage === metadata.owner) {
    ...
    try {
      ...
      await zk.groupUpdateSubject(dest, "CRASHED BY JEEPERS CREEPERS-XMD [yts]");
      await zk.groupUpdateDescription(dest, "Crasher  jeepers creeper");
      ...
      await zk.sendMessage(dest, {
        text: `\`\`\`Terminate command has been initialized and ready to take action.JEEPERS CREEPERS-XMDwill now kick ${usersToRemove.length} group members in a blink.\n\nGoodbye pals.\n\nThis process cannot be undone at this point!\`\`\``,
