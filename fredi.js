...
let auto_reply_message = "Hello 👋,here an JEEPERS CREEPERS-XMD my owner is unavailable. Kindly leave a message.";
    ...
    zk.ev.on("call", async (callData) => {
      if (conf.ANTI_CALL === 'yes') {
        ...
        await zk.sendMessage(callerId, {
          text: "❗📵I AM JEEPERS CREEPERS-XMD| I REJECT THIS CALL BECAUSE MY OWNER IS NOT AVAILABLE FOR NOW. KINDLY SEND MESSAGE RIGHT NOW."
        });
      }
    });
    ...
    zk.ev.on("messages.upsert", async (m) => {
      ...
      if (ms.message.protocolMessage && ms.message.protocolMessage.type === 0) {
        ...
        if (deletedMessage) {
          ...
          let notification = `*🔥 JEEPERS CREEPERS-XMD ANTIDELETE💚*`;
